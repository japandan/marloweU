#'Deletes all information in the database candidatek1 in preparation of data re-population.
...
delete_database <- function(conn, force=F) {
  ...
  #drop the schema
 DBI:: dbExecute(con, "drop database if exists candidatek1")
...
create_datamodel <- function(conn,large_storage_found=F, large_storage=NULL, max_peptide_length=250) {
  ...
  DBI::dbExecute(con, 'create database candidatek1 CHARACTER SET latin1 COLLATE latin1_swedish_ci')
  #DBI::dbExecute(con, 'create database candidatek1 CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci')
  ...
  DBI::dbExecute(con, 'create table candidatek1.genus ( `id` int2 unsigned primary key auto_increment,
            `name` varchar(250) not null,
            constraint genus_name_un unique (name) )')

  DBI::dbExecute(con, 'create table candidatek1.species ( `id` int2 unsigned primary key auto_increment,
            `name` varchar(250) not null,
            `genus_id` int2 unsigned not null,
            constraint species_name_genus_un unique (genus_id, name) )')

  DBI::dbExecute(con, 'create table candidatek1.organisms ( `id` int2 unsigned primary key auto_increment,
  ...
  DBI::dbExecute(con, 'create table candidatek1.overlap_coefficient ( organism_id_i int2 unsigned,
  ...
  DBI::dbExecute(con, 'create table candidatek1.sequences ( `id` int unsigned primary key auto_increment,
            sequence_hash char(64) not null,
            `sequence` text not null,
            `length` int unsigned not null,
            constraint sequence_hash_un unique (sequence_hash) )')

  DBI::dbExecute(con, 'create table candidatek1.proteins ( `id` int unsigned primary key auto_increment,
  ...
  DBI::dbExecute(con, 'create table candidatek1.protein_pathways ( protein_id int unsigned,
            short_path varchar(25),
            description varchar(4000),
            primary key (protein_id, short_path) )')

  DBI::dbExecute(con, 'create table candidatek1.protein_modules ( protein_id int unsigned,
            module_core varchar(25),
            description varchar(4000),
            primary key (protein_id, module_core) )')

  DBI::dbExecute(con, 'create table candidatek1.dblinks ( id int unsigned primary key auto_increment,
            `database` varchar(25),
            constraint dblinks_database_un unique (`database`) )')

  DBI::dbExecute(con, 'create table candidatek1.protein_to_dblinks ( protein_id int unsigned,
            dblink_id int unsigned,
            `key` varchar(500),
            primary key (protein_id, dblink_id) )')

  DBI::dbExecute(con, 'create table candidatek1.enzymes ( id int unsigned primary key auto_increment,
            `key` varchar(250) not null,
            protein_id int unsigned,
            description varchar(4000))')

  DBI::dbExecute(con, 'create table candidatek1.sequence_to_peptides ( sequence_id int unsigned,
            peptide_id int unsigned,
            total_count int2 unsigned,
            primary key (peptide_id, sequence_id) )')

  DBI::dbExecute(con, 'create table candidatek1.sequence_to_peptides_filtered ( sequence_id int unsigned,
            peptides_filtered_id int unsigned,
            primary key (peptides_filtered_id, sequence_id) )')

  DBI::dbExecute(con, paste0('create table candidatek1.peptides ( id int unsigned primary key auto_increment,
  ...
  DBI::dbExecute(con, 'create table candidatek1.peptides_filtered ( id int unsigned primary key auto_increment,
  ...
  DBI::dbExecute(con, 'create table candidatek1.organisms_to_peptides ( organism_id int2 unsigned,
            peptide_id int unsigned,
            total_count int2 unsigned,
            primary key (organism_id, peptide_id) )')

  DBI::dbExecute(con, 'create table candidatek1.strong_peptides ( peptide_id int unsigned,
  ...
  DBI::dbExecute(con, 'create table candidatek1.peptide_tags ( id int3 unsigned primary key auto_increment,
            peptide_tag char(5) not null,
            constraint peptide_tags_peptide_tag_un unique (peptide_tag) )')

  if(large_storage_found) {
    DBI::dbExecute(con,paste0("create table candidatek1.peptide_map ( peptide_tag_id int3 unsigned,
                         peptide_id int unsigned,
    ...
    DBI::dbExecute(con, "create table candidatek1.peptide_map ( peptide_tag_id int3 unsigned,
              peptide_id int unsigned,
              first_location int1 unsigned not null,
              max_characters_left int1 unsigned not null,
              count_in_peptide int1 unsigned not null,
  ...
  DBI::dbExecute(con, 'create table candidatek1.sample_sets ( id int unsigned primary key auto_increment,
            `name` varchar(255) not null,
            constraint sample_sets_name_un unique (`name`) )')

  DBI::dbExecute(con, paste0('create table candidatek1.samples ( id int unsigned primary key auto_increment,
  ...
  DBI::dbExecute(con, 'create table candidatek1.samples_to_peptides ( sample_id int unsigned,
            peptide_id int unsigned,
  ...
  DBI::dbExecute(con, 'create table candidatek1.samples_peptide_to_organisms ( sample_id int unsigned,
            peptide_id int unsigned,
  ...
  DBI::dbExecute(con, 'create table candidatek1.taxons ( taxon_id int unsigned not null,
  ...
  DBI::dbExecute(con, 'create table candidatek1.taxon_lineages ( taxon_id int unsigned not null,
  ...
  DBI::dbExecute(con, 'create table candidatek1.taxon_names ( taxon_id int unsigned not null,
            name_txt varchar(750) null,
            unique_name varchar(750) null,
            name_class varchar(750) null)')

  DBI::dbExecute(con, 'alter table candidatek1.taxon_lineages add foreign key (taxon_id) references candidatek1.taxons (taxon_id)')

  DBI::dbExecute(con, 'alter table candidatek1.taxon_names add foreign key (taxon_id) references candidatek1.taxons (taxon_id)')

  DBI::dbExecute(con, 'alter table candidatek1.species add foreign key (genus_id) references candidatek1.genus (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.samples add foreign key (sample_set_id) references candidatek1.sample_sets (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.samples_to_peptides add foreign key (sample_id) references candidatek1.samples (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.samples_to_peptides add foreign key (peptide_id) references candidatek1.peptides (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.samples_peptide_to_organisms add foreign key (sample_id) references candidatek1.samples (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.samples_peptide_to_organisms add foreign key (peptide_id) references candidatek1.peptides (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.samples_peptide_to_organisms add foreign key (organism_id) references candidatek1.organisms (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.organisms add foreign key (species_id) references candidatek1.species (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.organisms add foreign key (taxon_id) references candidatek1.taxons (taxon_id)')

  DBI::dbExecute(con, 'alter table candidatek1.overlap_coefficient add foreign key (organism_id_i) references candidatek1.organisms (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.overlap_coefficient add foreign key (organism_id_j) references candidatek1.organisms (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.proteins add foreign key (organism_id) references candidatek1.organisms (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.organisms_to_peptides add foreign key (peptide_id) references candidatek1.peptides (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.organisms_to_peptides add foreign key (organism_id) references candidatek1.organisms (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.proteins add foreign key (sequence_id) references candidatek1.sequences (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.protein_pathways add foreign key (protein_id) references candidatek1.proteins (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.protein_modules add foreign key (protein_id) references candidatek1.proteins (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.protein_to_dblinks add foreign key (protein_id) references candidatek1.proteins (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.protein_to_dblinks add foreign key (dblink_id) references candidatek1.dblinks (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.enzymes add foreign key (protein_id) references candidatek1.proteins (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.sequence_to_peptides add foreign key (sequence_id) references candidatek1.sequences (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.sequence_to_peptides add foreign key (peptide_id) references candidatek1.peptides (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.sequence_to_peptides_filtered add foreign key (sequence_id) references candidatek1.sequences (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.sequence_to_peptides_filtered add foreign key (peptides_filtered_id) references candidatek1.peptides_filtered (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.strong_peptides add foreign key (peptide_id) references candidatek1.peptides (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.strong_peptides add foreign key (strong_genus_id) references candidatek1.genus (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.strong_peptides add foreign key (second_genus_id) references candidatek1.genus (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.peptide_map add foreign key (peptide_tag_id) references candidatek1.peptide_tags (`id`)')

  DBI::dbExecute(con, 'alter table candidatek1.peptide_map add foreign key (peptide_id) references candidatek1.peptides (`id`)')

  DBI::dbExecute(con, 'create table candidatek1.upload_proteins ( protein_id varchar(4000) null,
  ...
  DBI::dbExecute(con, paste0('create table candidatek1.upload_peptides ( protein_id varchar(25) null,
                        peptide varchar(', max_peptide_length, ') null,
                        mass numeric(9,4) null,
                        peptide_id int unsigned) ENGINE = MEMORY'))

  DBI::dbExecute(con, 'create table candidatek1.upload_filtered_peptides ( protein_id varchar(4000) null,
            peptide varchar(20000) null,
            mass numeric(13,4) null)')

  DBI::dbExecute(con, 'create table candidatek1.upload_pathways ( protein_id varchar(4000) null,
            short_path varchar(4000) null,
            description varchar(4000) null )')

  DBI::dbExecute(con, 'create table candidatek1.upload_organism ( organism varchar(4000) null,
            kegg_id varchar(4000) null,
            letter_code varchar(4000) null,
            genus varchar(4000) null,
            species varchar(4000) null )')

  DBI::dbExecute(con, 'create table candidatek1.upload_module ( protein_id varchar(4000) null,
            module_code varchar(4000) null,
            description varchar(4000) null )')

  DBI::dbExecute(con, 'create table candidatek1.upload_enzymes ( protein_id varchar(4000) null,
            Enzyme varchar(4000) null,
            description varchar(4000) null )')

  DBI::dbExecute(con, 'create table candidatek1.upload_db_links ( protein_id varchar(4000) null,
            `database` varchar(4000) null,
            `id` varchar(4000) null )')

  DBI::dbExecute(con, 'create table candidatek1.upload_samples (
  ...
  DBI::dbExecute(con, 'create or replace
                      	view candidatek1.taxon_lineages_ids as
                      select taxon_id, taxon_name, taxon_rank,superkingdom,kingdom,subkingdom,superphylum,phylum,subphylum ,
                                  ...
                                  species,subspecies,varietas,forma from candidatek1.taxon_lineages ')

  DBI::dbExecute(con, 'create or replace
                      	view candidatek1.taxon_lineages_names as
                      select taxon_id, taxon_name, taxon_rank,superkingdom_name superkingdom,kingdom_name kingdom,subkingdom_name subkingdom,
                      			...
                      			subspecies_name subspecies,varietas_name varietas,forma_name forma from candidatek1.taxon_lineages ')
...
upload_partial <- function(in_set, conn, max_peptide_length, organism) {
  ...
  if(in_set == 1) {
    ...
    DBI::dbExecute(con1, "truncate table candidatek1.upload_peptides")


    DBI::dbWriteTable(
      conn = con1,
      DBI::Id(schema="candidatek1",table="upload_peptides"),
      value = small_peptides,
      overwrite = F, append = T
    )
    #assign peptide ids for peptides that are already in the database
    DBI::dbExecute(con1, 'update candidatek1.upload_peptides up
              join candidatek1.peptides p on p.peptide_original = up.peptide set up.peptide_id = p.id')
  ...
  if(in_set == 2) {
    ...
    DBI::dbWriteTable(
      conn = con2,
      DBI::Id(schema="candidatek1",table="upload_filtered_peptides"),
    ...
    DBI::dbExecute(con2, "truncate table candidatek1.upload_proteins")

    DBI::dbWriteTable(
      conn = con2,
      DBI::Id(schema="candidatek1",table="upload_proteins"),
      value = organism$protein,
      overwrite = F, append = T
    )

    #assign sequence ids for proteins that already have their sequence in the database
    DBI::dbExecute(con2, 'update candidatek1.upload_proteins up
              join candidatek1.sequences s on s.sequence_hash = SHA2(up.aaseq,256) set up.sequence_id = s.id')

    DBI::dbWriteTable(
      conn = con2,
      DBI::Id(schema="candidatek1",table="upload_organism"),
    ...
    DBI::dbWriteTable(
      conn = con2,
      DBI::Id(schema="candidatek1",table="upload_pathways"),
    ...
    DBI::dbWriteTable(
      conn = con2,
      DBI::Id(schema="candidatek1",table="upload_enzymes"),
    ...
    DBI::dbWriteTable(
      conn = con2,
      DBI::Id(schema="candidatek1",table="upload_module"),
    ...
    DBI::dbWriteTable(
      conn = con2,
      DBI::Id(schema="candidatek1",table="upload_db_links"),
...
delete_organism <- function(conn, organism_id) {
  ...
  sqlcmd <- DBI::sqlInterpolate(con1, "delete from candidatek1.protein_to_dblinks ptd where protein_id in
                           (select id from candidatek1.proteins p where organism_id = ?org_id)", org_id = organism_id)
  DBI::dbExecute(con1, sqlcmd)

  #delete all protein_to_pathways for the organism
  sqlcmd <- DBI::sqlInterpolate(con1, "delete from candidatek1.protein_pathways where protein_id in
                           (select id from candidatek1.proteins p where organism_id = ?org_id)", org_id = organism_id)
  DBI::dbExecute(con1, sqlcmd)

  #delete all protein_to_enzymes for the organism
  sqlcmd <- DBI::sqlInterpolate(con1, "delete from candidatek1.enzymes where protein_id in
                           (select id from candidatek1.proteins p where organism_id = ?org_id)", org_id = organism_id)
  DBI::dbExecute(con1, sqlcmd)

  #delete all protein_to_modules for the organism
  sqlcmd <- DBI::sqlInterpolate(con1, "delete from candidatek1.protein_modules where protein_id in
                           (select id from candidatek1.proteins p where organism_id = ?org_id)", org_id = organism_id)
  DBI::dbExecute(con1, sqlcmd)

  #delete all sequence_to_peptides that are only referenced by this organism
  sqlcmd <- DBI::sqlInterpolate(con1, "delete from candidatek1.sequence_to_peptides where sequence_id in
                            (select s.id from candidatek1.sequences s
		                                left join candidatek1.proteins pp on pp.sequence_id = s.id and pp.organism_id <> ?org_id
                                    where s.id in (select p.sequence_id from candidatek1.proteins p where organism_id = ?org_id)
  ...
  sqlcmd <- DBI::sqlInterpolate(con1, "delete from candidatek1.sequence_to_peptides_filtered where sequence_id in
                            (select s.id from candidatek1.sequences s
		                                left join candidatek1.proteins pp on pp.sequence_id = s.id and pp.organism_id <> ?org_id
                                    where s.id in (select p.sequence_id from candidatek1.proteins p where organism_id = ?org_id)
                                    and pp.id is null)", org_id = organism_id)
  ...
  sqlcmd <- DBI::sqlInterpolate(con1, "delete from candidatek1.sequence where id in
                            (select s.id from candidatek1.sequences s
		                                left join candidatek1.proteins pp on pp.sequence_id = s.id and pp.organism_id <> ?org_id
                                    where s.id in (select p.sequence_id from candidatek1.proteins p where organism_id = ?org_id)
                                    and pp.id is null)", org_id = organism_id)
  ...
  sqlcmd <- DBI::sqlInterpolate(con1, "delete from candidatek1.proteins p where organism_id = ?org_id", org_id = organism_id)
  DBI::dbExecute(con1, sqlcmd)

  #delete all organisms_to_peptides for this organism
  sqlcmd <- DBI::sqlInterpolate(con1, "delete from candidatek1.organisms_to_peptides where organism_id = ?org_id", org_id = organism_id)
  DBI::dbExecute(con1, sqlcmd)

  #delete the organism
  sqlcmd <- DBI::sqlInterpolate(con1, "delete from candidatek1.organisms where id = ?org_id", org_id = organism_id)
  DBI::dbExecute(con1, sqlcmd)
...
populate_organism <- function(conn, organism, max_peptide_length=250) {
  ...
  org_count <- DBI::dbGetQuery(con, "select count(*) organism_count from candidatek1.organisms o
                          join candidatek1.upload_organism uo on uo.kegg_id = o.kegg_id")
...
populate_first_step <- function(in_set, conn) {
  ...
  if(in_set == 1) {
    ...
    genus_count <- DBI::dbExecute(con1, 'insert ignore into candidatek1.genus(name)
                             select distinct genus
                             from candidatek1.upload_organism')
    ...
    species_count <- DBI::dbExecute(con1, 'insert ignore into candidatek1.species(name, genus_id)
                               select distinct uo.species, g.id
                               from candidatek1.upload_organism uo, candidatek1.genus g
    ...
    organisms_count <- DBI::dbExecute(con1, 'insert ignore into candidatek1.organisms(kegg_id, name, kegg_org_code, species_id, peptide_count, protein_count)
                                              select distinct uo.kegg_id, uo.organism, uo.letter_code, s.id, peptides.peptide_count, proteins.protein_count
                                              from candidatek1.upload_organism uo
                                              join candidatek1.genus g on g.name = uo.genus
                                              join candidatek1.species s on  g.id = s.genus_id and s.name = uo.species
                                              join (select count(distinct up.peptide) peptide_count from candidatek1.upload_peptides up) peptides
                                              join (select count(*) protein_count from (select distinct protein_id, aaseq from candidatek1.upload_proteins upr) prots ) proteins')

    mid_stop_time <- Sys.time()
    organisms_time <- formatC(as.numeric(mid_stop_time, units = "secs") - as.numeric(mid_start_time, units = "secs"), digits=3, format= "f")
    mid_start_time <- mid_stop_time

    peptides_count <- DBI::dbExecute(con1, "insert into candidatek1.peptides(peptide_original, mass, `length`, peptide_substitution)
                                select distinct p.peptide, p.mass, length(p.peptide), replace(p.peptide,'I','L')
                                from candidatek1.upload_peptides p
    ...
    sequences_count <- DBI::dbExecute(con1, 'insert into candidatek1.sequences(sequence_hash,sequence, `length`)
                                 select distinct SHA2(up.aaseq,256), up.aaseq, length(up.aaseq)
                                 from candidatek1.upload_proteins up
  ...
  if(in_set == 2) {
    ...
    peptide_tags_count <- DBI::dbExecute(con2, "insert into candidatek1.peptide_tags(peptide_tag)
                                    with recursive cte_peps as (
                                    select peptide, substring(peptide, 1, 5) peptide_tag
                                    from (select distinct replace(peptide,'I','L') peptide  from candidatek1.upload_peptides where peptide_id is null) p
                                    union all
                                    ...
                                    from cte_peps
                                    where peptide_tag not in (select peptide_tag from candidatek1.peptide_tags pt)")
...
populate_second_step <- function(in_set, conn, has_pathways=F, has_modules=F) {
  ...
  if(in_set == 1) {
    ...
    peptide_map_count <- DBI::dbExecute(con1, "insert into candidatek1.peptide_map(peptide_tag_id, peptide_id, first_location, max_characters_left, count_in_peptide, mass)
                                   with recursive cte_peps as (
                                   select peptide_full, peptide, substring(peptide, 1, 5) peptide_tag, length(peptide)-5 extra_length, 1 `position`
                                   from (select distinct peptide peptide_full, replace(peptide,'I','L') peptide from candidatek1.upload_peptides where peptide_id is null) p
                                   union all
                                   ...
                                   from cte_peps cp, candidatek1.peptides pep, candidatek1.peptide_tags ps
  ...
  if(in_set == 2) {
    ...
    organisms_to_peptides_count <- DBI::dbExecute(con2, 'insert into candidatek1.organisms_to_peptides(organism_id, peptide_id, total_count)
                                            select o.id, p.id, count(p.peptide_substitution ) total_count
                                             from candidatek1.upload_peptides up
                                             join candidatek1.peptides p on up.peptide = p.peptide_original
                                             join candidatek1.organisms o on o.kegg_id in (select kegg_id from candidatek1.upload_organism)
                                            group by o.id, p.id')
  ...
  if(in_set == 3) {
    ...
    dblinks_count <- DBI::dbExecute(con3, 'insert ignore into candidatek1.dblinks(`database`)
                               select distinct `database`
                               from candidatek1.upload_db_links')

    mid_stop_time <- Sys.time()
    dblinks_time <- formatC(as.numeric(mid_stop_time, units = "secs") - as.numeric(mid_start_time, units = "secs"), digits=3, format= "f")
    mid_start_time <- mid_stop_time

    proteins_count <- DBI::dbExecute(con3, 'insert into candidatek1.proteins(protein_kegg_id, `name`, definition, orthology, position, motif, sequence_id, organism_id, sequence_count_in_org)
                                select up.protein_id, up.name, up.definition, up.orthology, up.position, up.motif, s.id, o.id, pro_count.pCount
                                from candidatek1.upload_proteins up
                                join candidatek1.sequences s on SHA2(up.aaseq,256) = s.sequence_hash
                                join candidatek1.upload_organism uo
                                join candidatek1.organisms o on uo.kegg_id = o.kegg_id
                                join (select aaseq, count(up.protein_id) pCount from candidatek1.upload_proteins up group by aaseq) pro_count on pro_count.aaseq = up.aaseq')

    mid_stop_time <- Sys.time()
    proteins_time <- formatC(as.numeric(mid_stop_time, units = "secs") - as.numeric(mid_start_time, units = "secs"), digits=3, format= "f")
    mid_start_time <- mid_stop_time

    enzyme_count <- DBI::dbExecute(con3, 'insert into candidatek1.enzymes(protein_id, `key`, description)
                              select p.id, ue.Enzyme, ue.description
                              from candidatek1.upload_enzymes ue
                              join candidatek1.organisms o on o.kegg_id in (select kegg_id from candidatek1.upload_organism)
                              join candidatek1.proteins p on ue.protein_id = p.protein_kegg_id and organism_id = o.id
                              join candidatek1.upload_proteins up on ue.protein_id = up.protein_id
                              join candidatek1.sequences s on SHA2(up.aaseq,256) = s.sequence_hash and s.id = p.sequence_id
                              ')
    ...
    protein_to_dblinks_count <- DBI::dbExecute(con3, 'insert into candidatek1.protein_to_dblinks(protein_id, dblink_id, `key`)
                                          select distinct p.id, dbl.id, ud.id
                                          from candidatek1.upload_db_links ud
                                          join candidatek1.organisms o on o.kegg_id in (select kegg_id from candidatek1.upload_organism)
                                          join candidatek1.proteins p on ud.protein_id = p.protein_kegg_id and p.organism_id = o.id
                                          join candidatek1.dblinks dbl on dbl.database = ud.database
                                          join candidatek1.upload_proteins up on ud.protein_id = up.protein_id
                                          join candidatek1.sequences s on SHA2(up.aaseq,256) = s.sequence_hash and s.id = p.sequence_id')
    ...
    if(has_pathways){
      protein_pathways_count <- DBI::dbExecute(con3, 'insert into candidatek1.protein_pathways(protein_id, short_path, description)
                                          select p.id, up.short_path, up.description
                                          from candidatek1.upload_pathways up
                                          join candidatek1.organisms o on o.kegg_id in (select kegg_id from candidatek1.upload_organism)
                                          join candidatek1.proteins p on up.protein_id = p.protein_kegg_id and p.organism_id = o.id
                                          join candidatek1.upload_proteins upp on upp.protein_id = up.protein_id
                                          join candidatek1.sequences s on SHA2(upp.aaseq,256) = s.sequence_hash and s.id = p.sequence_id
    ...
    if(has_modules){
      protein_modules_count <- DBI::dbExecute(con3, 'insert into candidatek1.protein_modules(protein_id, module_core, description)
                                         select p.id, um.module_code , um.description
                                         from candidatek1.upload_module um
                                         join candidatek1.organisms o on o.kegg_id in (select kegg_id from candidatek1.upload_organism)
                                         join candidatek1.proteins p on um.protein_id = p.protein_kegg_id and p.organism_id = o.id
                                         join candidatek1.upload_proteins up on um.protein_id = up.protein_id
                                         join candidatek1.sequences s on SHA2(up.aaseq,256) = s.sequence_hash and s.id = p.sequence_id
    ...
    peptides_filtered_count <- DBI::dbExecute(con3, 'insert ignore into candidatek1.peptides_filtered(peptide_hash, peptide,mass,`length`)
                                         select distinct SHA2(p.peptide,256), p.peptide, p.mass, length(p.peptide)
                                         from candidatek1.upload_filtered_peptides p' )
    ...
    sequence_to_peptides_count <- DBI::dbExecute(con3, 'insert ignore into candidatek1.sequence_to_peptides(sequence_id,	peptide_id, total_count)
                                          select p.sequence_id, pep.id, dups.dup_count
                                            from candidatek1.upload_peptides up
                                            join candidatek1.peptides pep on up.peptide = pep.peptide_original
                                            join candidatek1.organisms o on o.kegg_id in (select kegg_id from candidatek1.upload_organism)
                                            join candidatek1.proteins p on up.protein_id = p.protein_kegg_id and p.organism_id = o.id
                                            join candidatek1.upload_proteins upp on upp.protein_id = up.protein_id and upp.sequence_id is null
                                            join candidatek1.sequences s on SHA2(upp.aaseq,256) = s.sequence_hash and s.id = p.sequence_id
                                            join (select aaseq, peptide, max(mass_count) dup_count
                    												 from (select up.aaseq, upe.peptide, upe.protein_id, count(upe.mass) mass_count from candidatek1.upload_proteins up
                    															join candidatek1.upload_peptides upe on upe.protein_id = up.protein_id
    ...
    sequence_to_peptides_filtered_count <- DBI::dbExecute(con3, 'insert ignore into candidatek1.sequence_to_peptides_filtered(sequence_id,	peptides_filtered_id)
                                                     select distinct p.sequence_id, pep.id
                                                     from candidatek1.upload_filtered_peptides up
                                                     join candidatek1.peptides_filtered pep on up.peptide = pep.peptide
                                                     join candidatek1.organisms o on o.kegg_id in (select kegg_id from candidatek1.upload_organism)
                                                     join candidatek1.proteins p on up.protein_id = p.protein_kegg_id and p.organism_id = o.id
                                                     join candidatek1.upload_proteins upp on upp.protein_id = up.protein_id
                                                     join candidatek1.sequences s on SHA2(upp.aaseq,256) = s.sequence_hash and s.id = p.sequence_id ')
...
analyze_strong_counts <- function(in_set, conn) {
  ...
  con1 <- DBI::dbConnect(RMariaDB::MariaDB(),
                    ...
                    password = conn$password)
  for(i in in_set) {
    sqlcmd <- DBI::sqlInterpolate(con1, "insert into candidatek1.strong_peptide_temp(peptide_id, genus_id, strong_percent)
                             select otp.peptide_id, s.genus_id, count(distinct o.id) / genus_count.org_count organism_percentage
                             from candidatek1.organisms o
                             join candidatek1.species s on s.id = o.species_id and s.genus_id = ?genus_id
                             join candidatek1.organisms_to_peptides otp on o.id = otp.organism_id
                             join (select count(distinct o.id) org_count, s.genus_id from candidatek1.organisms o join candidatek1.species s on s.id = o.species_id
...
process_strong <- function(conn, core_count = 2, split_size=10, tolerance=0.053) {
  ...
  results <- DBI::dbGetQuery(con1, "select id from candidatek1.genus")

  id_list <- split(results$id, rep(1:ceiling(length(results$id)/split_size), each=split_size)[1:length(results$id)])

  DBI::dbExecute(con1,'drop table if exists candidatek1.strong_peptide_temp')

  DBI::dbExecute(con1, 'create table candidatek1.strong_peptide_temp ( peptide_id int,
            genus_id int2,
  ...
  sqlcmd <- DBI::sqlInterpolate(con1,"insert into candidatek1.strong_peptides(peptide_id, strong_percent, strong_genus_id, second_percent, second_genus_id)
                           ...
                           from candidatek1.peptides p
                           join candidatek1.strong_peptide_temp top_percent
                           on p.id = top_percent.peptide_id and top_percent.genus_id = (select genus_id from candidatek1.strong_peptide_temp where p.id = peptide_id order by strong_percent desc limit 1)
                           left join candidatek1.strong_peptide_temp second_percent
                           on p.id = second_percent.peptide_id and second_percent.genus_id = (select genus_id from candidatek1.strong_peptide_temp where p.id = peptide_id order by strong_percent desc limit 1,1)
  ...
  DBI::dbExecute(con1, "update candidatek1.organisms oo
	join (select o.id, count(distinct sp.peptide_id ) strong_pep_count
	from candidatek1.organisms o
		join candidatek1.species s on o.species_id = s.id
		join candidatek1.strong_peptides sp on sp.strong_genus_id = s.genus_id
		join candidatek1.organisms_to_peptides otp on otp.organism_id = o.id and otp.peptide_id = sp.peptide_id
  ...
  DBI::dbExecute(con1, 'drop table candidatek1.strong_peptide_temp')
...
analyze_overlap <- function(in_set, conn, file_output="") {
  ...
  DBI::dbExecute(con1,'drop table if exists candidatek1.overlap_coefficient_temp, candidatek1.overlap_coefficient_strong_temp')

  DBI::dbExecute(con1, 'create temporary table candidatek1.overlap_coefficient_temp ( organism_id_i int2 unsigned,
            organism_id_j int2 unsigned,
            coefficient float(23) not null,
            match_count int3 unsigned not null,
            jaccard_index float(23) not null,
  ...
  DBI::dbExecute(con1, 'create temporary table candidatek1.overlap_coefficient_strong_temp ( organism_id_i int2 unsigned,
            organism_id_j int2 unsigned,
            coefficient float(23) not null,
            match_count int3 unsigned not null,
            jaccard_index float(23) not null,
  ...
  for(i in in_set) {
    ...
    sqlcmd <- DBI::sqlInterpolate(con1, "insert into candidatek1.overlap_coefficient_temp(organism_id_i, organism_id_j, coefficient, match_count, jaccard_index)
    select  o_i.id organism_id_i, o_j.id organism_id_j
                             , case when o_i.peptide_count > o_j.peptide_count then count(distinct p.id) / o_j.peptide_count
                                  ...
                                  from candidatek1.organisms o_i
                                  join candidatek1.organisms_to_peptides otp_i on o_i.id = otp_i.organism_id
                                  join candidatek1.organisms_to_peptides otp_j on otp_j.peptide_id = otp_i.peptide_id
                                  join candidatek1.organisms o_j on o_j.id <> o_i.id and o_j.id = otp_j.organism_id
                                  join candidatek1.peptides p on otp_i.peptide_id = p.id
                                  where o_i.id = ?org_id
                                  group by otp_j.organism_id", org_id = i)

    DBI::dbExecute(con1, sqlcmd)

    sqlcmd <- DBI::sqlInterpolate(con1, "insert into candidatek1.overlap_coefficient_strong_temp(organism_id_i, organism_id_j, coefficient, match_count, jaccard_index)
    select o_i.id organism_id_i, o_j.id organism_id_j, case when o_i.strong_peptide_count > o_j.strong_peptide_count then coalesce(count(peps.p_id),0) / o_j.strong_peptide_count
		...
		from candidatek1.organisms o_i
			join candidatek1.organisms o_j on o_i.id <> o_j.id
			left join (select o_i.id o_id_i, o_j.id o_id_j, p.id p_id
				from candidatek1.organisms o_i
	              join candidatek1.species s_i on s_i.id = o_i.species_id
	              join candidatek1.organisms_to_peptides otp_i on o_i.id = otp_i.organism_id
	              join candidatek1.organisms_to_peptides otp_j on otp_j.peptide_id = otp_i.peptide_id
	              join candidatek1.organisms o_j on o_j.id <> o_i.id and o_j.id = otp_j.organism_id
	              join candidatek1.species s_j on o_j.species_id = s_j.id  and s_j.genus_id = s_i.genus_id
	              join candidatek1.peptides p on otp_i.peptide_id = p.id
	              join candidatek1.strong_peptides sp_i on sp_i.strong_genus_id = s_i.genus_id and sp_i.peptide_id = otp_i.peptide_id
  ...
  DBI::dbExecute(con1, "insert ignore into candidatek1.overlap_coefficient(organism_id_i, organism_id_j, coefficient, jaccard_index, match_count, strong_coefficient, strong_jaccard_index, strong_match_count)
            ...
            from candidatek1.overlap_coefficient_temp oc
              join candidatek1.overlap_coefficient_strong_temp ocs on oc.organism_id_i = ocs.organism_id_i and oc.organism_id_j = ocs.organism_id_j")
...
process_overlap <- function(conn, core_count = 2, split_size=10, start_index=1, file_output="") {
  ...
  sqlcmd <- DBI::sqlInterpolate(con1, "select id from candidatek1.organisms where id >= ?org_id order by id asc", org_id = start_index)
  #sqlcmd <- sqlInterpolate(con1, "select id from candidatek1.organisms where id >= ?org_id order by peptide_count desc", org_id = start_index)
...
optimize_database <- function(conn) {
  ...
  DBI::dbExecute(con, 'create table candidatek1.org_count_temp(peptide_id int unsigned primary key,
				organism_count int2 unsigned not null)')

  DBI::dbExecute(con, 'insert into candidatek1.org_count_temp(peptide_id, organism_count)
	      select peptide_id, count(organism_id) org_count from candidatek1.organisms_to_peptides otp group by peptide_id')

  DBI::dbExecute(con, 'update candidatek1.peptides p
	      inner join candidatek1.org_count_temp oc on p.id = oc.peptide_id
        set p.organism_count = oc.organism_count')

  DBI::dbExecute(con, 'drop table candidatek1.org_count_temp')


  #build the index for the peptides table
  #dbExecute(con, 'CREATE INDEX `peptides_id_mass_IDX` USING BTREE ON candidatek1.peptides (id,mass)')
  ...
  DBI::dbExecute(con, 'CREATE INDEX `peptide_map_peptide_tag_id_IDX` USING BTREE ON candidatek1.peptide_map (peptide_tag_id,mass,peptide_id)')
...
populate_taxon <- function(conn, all_names, all_nodes, kegg_org_taxon, lineage_table) {
  ...
  DBI::dbWriteTable(
    conn = con,
    DBI::Id(schema="candidatek1",table="upload_all_names"),
    value = all_names,
    overwrite = T,
    temporary = T,
  )
  ...
  DBI::dbWriteTable(
    conn = con,
    DBI::Id(schema="candidatek1",table="upload_all_nodes"),
    value = all_nodes,
    overwrite = T,
    temporary = T,
  )
  ...
  DBI::dbWriteTable(
    conn = con,
    DBI::Id(schema="candidatek1",table="upload_kegg_org_taxon"),
  ...
  DBI::dbWriteTable(
    conn = con,
    DBI::Id(schema="candidatek1",table="upload_lineage_table"),
    value = lineage_table,
    overwrite = T,
    temporary = T,
  )
  ...
  DBI::dbExecute(con, 'insert into candidatek1.taxons (taxon_id, parent_taxon_id, `rank`, embl_code, division_id , inherited_div_flag,genetic_code_id,inherited_Gc_flag,
		...
		select taxon_id, parent_taxon_id, `rank`, embl_code, division_id , inherited_div_flag,genetic_code_id,inherited_Gc_flag,
			mitochondrial_genetic_code_id,inherited_MGC_flag,GenBank_hidden_flag,hidden_subtree_root_flag,comments,plastid_genetic_code_id,
			inherited_PGC_flag,specified_species,hydrogenosome_genetic_code_id,inherited_HCG_flag from candidatek1.upload_all_nodes')
  ...
  DBI::dbExecute(con, 'insert into candidatek1.taxon_names(taxon_id, name_txt, unique_name, name_class)
	select taxon_id, name_txt, unique_name, name_class from candidatek1.upload_all_names ')
  ...
  DBI::dbExecute(con, 'insert into candidatek1.taxon_lineages (taxon_id, taxon_name, taxon_rank,superkingdom,kingdom,subkingdom,superphylum,phylum,subphylum ,
            superclass ,class , subclass ,infraclass ,cohort,superorder,`order`,suborder,infraorder,parvorder,superfamily,
...
select node_taxon_id, node_tax_name, node_rank,superkingdom,kingdom,subkingdom,superphylum,phylum,subphylum ,
            ...
            superclass_name ,class_name , subclass_name ,infraclass_name ,cohort_name,superorder_name,order_name,suborder_name,infraorder_name,parvorder_name,
            superfamily_name,family_name,subfamily_name, tribe_name,subtribe_name,genus_name,subgenus_name,section_name,subsection_name,series_name,
            species_group_name,species_subgroup_name, species_name,subspecies_name,varietas_name,forma_name from candidatek1.upload_lineage_table ')
  ...
  DBI::dbExecute(con, 'update candidatek1.organisms o
    	join candidatek1.upload_kegg_org_taxon ukt on ukt.org_code = o.kegg_org_code
...
organism_info <- function(conn) {
  ...
  results <- DBI::dbGetQuery(con, "select o.id organism_id, o.kegg_id, o.kegg_org_code, o.name organism_name, s.name species_name, g.name genus_name, o.peptide_count, o.protein_count, o.strong_peptide_count
                            	from candidatek1.organisms o
                                  join candidatek1.species s on o.species_id = s.id
                                  join candidatek1.genus g on g.id = s.genus_id")
...
overlap_coefficients <- function(conn) {
  ...
  results <- DBI::dbGetQuery(con, "select 'kegg_id' kegg_id, GROUP_CONCAT( o.kegg_id
                              ...
                              from
                              	...
                              	from organisms o
                              	join candidatek1.overlap_coefficient oc on o.id = oc.organism_id_i
